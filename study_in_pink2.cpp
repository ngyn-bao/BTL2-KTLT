...
BaseItem *Robot::getKeepItem() const
{
    return item;
}
...
{
    if (obj->getCurrentPosition().isEqual(robot->getCurrentPosition().getRow(), robot->getCurrentPosition().getCol()))
    {
        if (canUse(obj, robot))
        {
            obj->setExp(ceil(obj->getExp() + (float)25 / 100 * obj->getExp()));
            obj->setExp(min(obj->getExp(), 900));
        }
    }
    else
        robot = nullptr;
...
void EnergyDrink::use(Character *obj, Robot *robot)
{
    if (obj->getCurrentPosition().isEqual(robot->getCurrentPosition().getRow(), robot->getCurrentPosition().getCol()))
    {
        if (canUse(obj, robot))
        {
            obj->setHp(ceil(obj->getHp() + (float)20 / 100 * obj->getHp()));
            obj->setHp(min(obj->getHp(), 500));
        }
    }
    else
        robot = nullptr;
...
void FirstAid::use(Character *obj, Robot *robot)
{
    ...
    if (canUse(obj, robot))
    {
        obj->setHp(ceil((float)obj->getHp() + (float)50 / 100 * (float)obj->getHp()));
        obj->setHp(min(obj->getHp(), 500));
    }
}
        ...
        if (sherlock)
        {
            ...
            if (RobotS *robotS = dynamic_cast<RobotS *>(robot))
            {
                ...
                else
                {
                    ...
                    {
                        sherlock->setExp(ceil((float)sherlock->getExp() - (float)sherlock->getExp() * (float)10 / 100));
                        sherlock->setExp(max(0, sherlock->getExp()));
                    }
                }
            }
            ...
            else if (RobotSW *robotSW = dynamic_cast<RobotSW *>(robot))
            {
                ...
                else
                {
                    ...
                    {
                        sherlock->setHp(ceil((float)sherlock->getHp() - (float)sherlock->getHp() * (float)15 / 100));
                        sherlock->setHp(max(0, sherlock->getHp()));
                        sherlock->setExp(ceil((float)sherlock->getExp() - (float)sherlock->getExp() * (float)15 / 100));
                        sherlock->setExp(max(0, sherlock->getExp()));
                    }
                }
            }
        ...
        else if (watson)
        {
            ...
            else if (RobotW *robotW = dynamic_cast<RobotW *>(robot))
            {
                ...
                else
                {
                    ...
                    {
                        watson->setHp((float)watson->getHp() - (float)watson->getHp() * (float)5 / 100);
                        watson->setHp(max(0, watson->getHp()));
                    }
                }
            }
            ...
            else if (RobotSW *robotSW = dynamic_cast<RobotSW *>(robot))
            {
                ...
                else
                {
                    ...
                    {
                        watson->setHp(ceil((float)watson->getHp() - (float)watson->getHp() * (float)15 / 100));
                        watson->setHp(max(0, watson->getHp()));
                        watson->setExp(ceil((float)watson->getExp() - (float)watson->getExp() * (float)15 / 100));
                        watson->setExp(max(0, watson->getExp()));
                    }
                }
            }
